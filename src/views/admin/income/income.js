                ...
                {allInput.map((ai) => (
                    <tr key={ai.id}>
                        ...
                        <td style={{textAlign: "center", display: "flex", justifyContent: "center"}}><MdEdit
                            onClick={() => handEditInputChange(ai.id, ai.money,ai.title)} style={{color:"red"}}/>
                            {editingRowId == ai.id ? (<MdSave style={{marginLeft: "15px" , color:"blue"}} onClick={()=>PostInputInUser()}/>) : <></>}</td>
                    </tr>
                ))}
                </tbody>
            </table>
